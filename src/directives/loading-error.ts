const icon = `
<svg class="h-32 w-32 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
const createNode = () => {
  ...
  div.setAttribute('data-name', 'mr-loading-error');
  div.className = 'mr-loader';
  const svgLoader = new DOMParser().parseFromString(icon, 'application/xml');
  div.appendChild(
...
export default {
  ...
  updated(el: HTMLElement, binding: { value: boolean }): void {
    const child = el.parentElement?.querySelector('[data-name=mr-loading-error]');
